...
double velprofile[] = {3, 4, 2.5, 3, 3, 3, 3};
double accprofile[] = {200, 200, 200, 200, 100, 100, 100};
...
void motion_callback(const ethercat_test::pos& msg)
{
    for (int i=0; i<NUMOFEPOS4_DRIVE; i++)
    {
        ...
        epos4_drive_pt[i].ptOutParam->ProfileVelocity = msg.position[7];
        epos4_drive_pt[i].ptOutParam->ProfileAcceleration = msg.position[8];
        epos4_drive_pt[i].ptOutParam->ProfileDeceleration = msg.position[8];
