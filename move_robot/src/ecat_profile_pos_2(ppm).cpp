...
double accprofile[] = {50, 50, 80, 80, 100, 100, 100};
...
//                    INT32	PositionDemandValue;		//0x6062
                    os=sizeof(ob); ob = 0x60620020;
                    wkc_count=ec_SDOwrite(k+1, 0x1A00, 0x07, FALSE, os, &ob, EC_TIMEOUTRXM);
                    ...
                    // 4. ProfileVelocity UINT32
                    os=sizeof(ob); ob = 0x60810020;
                    wkc_count=ec_SDOwrite(k+1, 0x1600, 0x04, FALSE, os, &ob, EC_TIMEOUTRXM);
                    // 5. ProfileAcceleration UINT32
                    os=sizeof(ob); ob = 0x60830020;
                    wkc_count=ec_SDOwrite(k+1, 0x1600, 0x05, FALSE, os, &ob, EC_TIMEOUTRXM);
                    // 6. ProfileDeceleration UINT32
                    os=sizeof(ob); ob = 0x60840020;
                    ...
                    os=sizeof(ob3); ob3 = 0x01;//ob3 = mode;
...
//    for (i=0; i<NUMOFEPOS4_DRIVE; i++)
//    {
//        epos4_drive_pt[i].ptOutParam->ControlWord=15; //Servo OFF (Disable voltage, transition#9)
//    }
//    ec_send_processdata();
//    wkc = ec_receive_processdata(EC_TIMEOUTRET);
//
...
//                {
int i=0;
//                rt_printf("EPOS4_DRIVE mode%i\n", epos4_drive_pt[i].ptInParam->ModeOfOperationDisplay);

                rt_printf("EPOS4_DRIVE #%i\n", i+1);
                    rt_printf("Statusword = 0x%x\n", epos4_drive_pt[i].ptInParam->StatusWord);
                    rt_printf("Actual Position = %i / %i\n" , epos4_drive_pt[i].ptInParam->PositionActualValue, epos4_drive_pt[i].ptInParam->PositionDemandValue);
                 i=1;
//                rt_printf("EPOS4_DRIVE mode%i\n", epos4_drive_pt[i].ptInParam->ModeOfOperationDisplay);
...
int i=0;
//        if (i==5){
            desinc[i] = int (msg.position[0]*262144/360) + homepos[i]; //degree
            epos4_drive_pt[i].ptOutParam->TargetPosition = desinc[i];
            epos4_drive_pt[i].ptOutParam->ProfileVelocity = msg.position[5];
            epos4_drive_pt[i].ptOutParam->ProfileAcceleration = msg.position[6];
            epos4_drive_pt[i].ptOutParam->ProfileDeceleration = msg.position[6];
    epos4_drive_pt[i].ptOutParam->ControlWord = 31;
    i=1;
    desinc[i] = int (msg.position[1]*262144/360) + homepos[i]; //degree
    epos4_drive_pt[i].ptOutParam->TargetPosition = desinc[i];
    epos4_drive_pt[i].ptOutParam->ProfileVelocity = msg.position[5];
    epos4_drive_pt[i].ptOutParam->ProfileAcceleration = msg.position[6];
    epos4_drive_pt[i].ptOutParam->ProfileDeceleration = msg.position[6];
    epos4_drive_pt[i].ptOutParam->ControlWord = 31;
    i=3;
    desinc[i] = int (msg.position[2]*262144/360) + homepos[i]; //degree
    epos4_drive_pt[i].ptOutParam->TargetPosition = desinc[i];
    epos4_drive_pt[i].ptOutParam->ProfileVelocity = msg.position[5];
    epos4_drive_pt[i].ptOutParam->ProfileAcceleration = msg.position[6];
    epos4_drive_pt[i].ptOutParam->ProfileDeceleration = msg.position[6];
    epos4_drive_pt[i].ptOutParam->ControlWord = 31;
    i=5;
    desinc[i] = int (msg.position[3]*262144/360) + homepos[i]; //degree
    epos4_drive_pt[i].ptOutParam->TargetPosition = desinc[i];
    epos4_drive_pt[i].ptOutParam->ProfileVelocity = msg.position[5];
    epos4_drive_pt[i].ptOutParam->ProfileAcceleration = msg.position[6];
    epos4_drive_pt[i].ptOutParam->ProfileDeceleration = msg.position[6];
    ...
    rt_printf("%i, targetpos = %i,%i\n" ,i, desinc[i],homepos[i]);
    ec_send_processdata();
    wkc = ec_receive_processdata(EC_TIMEOUTRET);

//        }
//        else{
//            desinc[i] = -int (msg.position[i]*262144/360) + homepos[i]; //degree
//            epos4_drive_pt[i].ptOutParam->TargetPosition = desinc[i];
